...
struct Tabella{
	...
	struct Lezione* lezioni; //puntatore di lezioni per il nostro orario
};
...
struct Tabella allocLezioni(struct Tabella t); //alloca memoria al puntatore di lezioni del nuovo orario
void deallocLezioni(struct Tabella * t); //dealloca memoria al puntatore di lezioni
...
struct Tabella allocLezioni(struct Tabella t){
	int dim = t.nRighe*t.nColonne;
	t.lezioni=(struct Lezione *) malloc(dim*sizeof(struct Lezione));
	return t;
}

void deallocLezioni(struct Tabella * t){
	free(t->lezioni);
	return;
}
