...
SELECT idCategoria, COUNT(*) AS numeroInserzioni
...
SELECT idTestata, COUNT(idInserzione) as numeroInserzioni
FROM instest
GROUP BY idTestata
ORDER BY idTestata;

-- 83 Visualizzare in quante testate è presente ogni inserzione
SELECT idInserzione, COUNT(DISTINCT idtestata) as numeroTestate
FROM instest
...
SELECT citta, COUNT(*) AS numero_privati
FROM citta LEFT JOIN privati ON privati.CAP = citta.CAP
WHERE citta <> 'Putignano'
GROUP BY citta
ORDER BY citta;

-- 87 Visualizzare i privati di Bari ordinati per nome
SELECT cognome, nome
FROM privati LEFT JOIN citta ON privati.CAP = citta.CAP
WHERE citta = "Bari"
ORDER BY nome;

-- 88 Visualizzare l'età media dei privati raggruppati per nome
SELECT nome, AVG(eta) AS etaMedia
FROM privati
GROUP BY nome;
...
SELECT idInserzione, insaz.idAzienda, referente
FROM insaz JOIN aziende ON insaz.idAzienda = aziende.idAzienda;
...
SELECT idInserzione, insaz.idAzienda, referente, telefono, citta
FROM insaz JOIN aziende ON insaz.idAzienda = aziende.idAzienda
...
SELECT idInserzione, inspriv.idPrivato, CONCAT(cognome, " ", nome) as privato
FROM inspriv  JOIN privati ON inspriv.idPrivato = privati.idPrivato;
...
SELECT t.nome, COUNT(ia.idInserzione) AS numeroInserzioni
FROM testate t
JOIN instest it ON t.idTestata = it.idTestata
JOIN insaz ia ON it.idInserzione = ia.idInserzione
JOIN aziende a ON ia.idAzienda = a.idAzienda
WHERE t.idTestata = (
    SELECT te.idTestata as test
    FROM instest te
    GROUP BY test
    ORDER BY COUNT(idInserzione) DESC
    LIMIT 1
)
GROUP BY t.nome;