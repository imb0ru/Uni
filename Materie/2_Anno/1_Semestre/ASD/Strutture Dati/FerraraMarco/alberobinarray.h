...
class alberoBinArray : public alberoBinAstratto<T, int>{
   public:
	...
	int contaFoglieSuLivello(int, posizione);

	bool partial_complete(posizione) const;
...
template <class T>
bool alberoBinArray<T>::partial_complete(posizione p) const {
    if (p == NIL) return true;

    if (!sinistroVuoto(p) || !destroVuoto(p)) {
        if (sinistroVuoto(p) != destroVuoto(p)) return false;

        return partial_complete(figlioSinistro(p)) && partial_complete(figlioDestro(p));
    }
...
template <class T>
int alberoBinArray<T>::contaFoglieSuLivello(int livello, posizione p) {
	// Se l'albero è vuoto o il livello è negativo, non ci sono foglie
    if (p == NIL || livello < 0) {
        return 0;
    }

	// Se è arrivato al livello corretto, controlla se è una foglia
    if (livello == 0) {
		// Se il nodo è una foglia, ritorna 1
        if (sinistroVuoto(p) && destroVuoto(p)) {
            return 1;
		// Altrimenti il nodo non è una foglia, ritorna 0
        } else {
            return 0;
        }
    }

	// Se non è arrivato al livello corretto, controlla i figli
    int foglieSinistro = contaFoglieSuLivello(livello - 1, figlioSinistro(p));
    int foglieDestro = contaFoglieSuLivello(livello - 1, figlioDestro(p));

	// Ritorna il numero di foglie trovate
    return foglieSinistro + foglieDestro;
}
