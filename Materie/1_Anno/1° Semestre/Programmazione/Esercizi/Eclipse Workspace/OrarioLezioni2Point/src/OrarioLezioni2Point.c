...
typedef struct Tabella{
	...
	lezione ** lezioni; //array bidimensionale per il nostro orario
}tabella;
...
tabella allocTabella(tabella t){
	int i=0;
	t.lezioni = (lezione **)malloc(leggereNRighe(t)*sizeof(lezione *));
	while(i<leggereNRighe(t)){
...
void consultaOrarioPerGiorno(tabella t){
	...
	while(r<leggereNRighe(t)){
		stampaLezione(leggereLezione(t,r,c));
		r++;
	}
}
